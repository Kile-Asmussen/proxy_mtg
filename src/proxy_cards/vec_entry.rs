...
pub trait VecEntryMethods<'a, K, V> {
    fn key(&self) -> &K;
    ...
    fn or_insert_with_key<F>(&mut self, closure: F) -> &mut V
    where
        F: FnOnce(&K) -> V;

    fn or_insert(&mut self, value: V) -> &mut V {
        self.or_insert_with(|| value)
    }

    fn or_insert_with<F>(&mut self, value: F) -> &mut V
    where
        F: FnOnce() -> V,
    {
...
impl<'a, K, V> VecEntryMethods<'a, K, V> for VecEntry<'a, K, V> {
    ...
    {
        match self {
            Self::Occupied(ove) => ove.and_modify(closure),
            Self::Vacant(vve) => vve.and_modify(closure),
        }
    }
...
impl<'a, K, V> VecEntryMethods<'a, K, V> for VacantVecEntry<'a, K, V> {
    ...
    where
        F: FnOnce(&K) -> V,
    {
        self.insert_entry(closure(self.key))
    }
}
